...
class App {
    ...
    private initControllers = (controllers: Interfaces.Controller[]) => {
        controllers.forEach((controller: Interfaces.Controller) => {
            this.app.use("/v1/messages", controller.router);
        });

        /** To check if server is running */
        this.app.use("/v1/messages/status", (req: express.Request, res: express.Response, next: express.NextFunction) => {
    ...
    private onListening = () => {
        ...
        console.log("Message service listening on " + bind);

        Socket.startSocket(this.port);
    };
